...
//xDelaunay Mesher(const std::vector<std::vector<std::vector<ThreeVector>>>& Position, ThreeVector<unsigned long> DetectorResolution)
//{
//    // Create a vector with a std::pair< Point, PointInfo (also a pair) > this is the input format for the Delaunay constructor
...
//    for(int nx = 0; nx < DetectorResolution.at(0); nx++)
//    {
//        for(int ny = 0; ny < DetectorResolution.at(1); ny++)
//        {
//            for(int nz = 0; nz < DetectorResolution.at(2); nz++) {
//
//                // Convert ThreeVector??????<float> of sample position to CGAL point
//                xPoint SamplePoint = xPoint(Position.at(0), Position.at(1), Position.at(2));
//
//                // Prepare point info (pair with track number and track sample number)
...
////Attention! the typedef of EMAP Delaunay related template is with x
//ThreeVector<float> EInterpolateCGAL(const std::vector<std::vector<std::vector<ThreeVector>>>& Position, const xDelaunay& Mesh, ThreeVector<float> Location)
//{
//    // Create a array which contains the info of all 4 vertices of a cell
...
//
//    // Find cell in the mesh where the point is located
//    xDelaunay::Cell_handle Cell =  Mesh.locate(VectorToPoint(Location));
//
//    // Loop over all four vertex points of the cell of interest
//    for(unsigned vertex_no = 0; vertex_no < PointIndex.size(); vertex_no++)
//    {
...
//            TransMatrix[row][column] = Position[PointIndex[column][0]][PointIndex[column][1]][PointIndex[column][2]].at(row) - Position[PointIndex[3][0]][PointIndex[3][1]][PointIndex[3][2]].at(row);
//        }
//    }
//
//    // Reuse Location and store its position relative to the last vertex of the cell it is contained in
//    // r - r4
//    Location -= Position[PointIndex[3][0]][PointIndex[3][1]][PointIndex[3][2]];
//
//    // If the transformation matrix can be successfully inverted
...
//        InterpolatedEfield = {0.0,0.0,0.0};
...
//    {
//        // Set E field to zero and end function immediately!
//        std::cout<<"The barycentric coordinate goes wrong at this E grid point. "<<std::endl;
//        InterpolatedEfield = {0.0,0.0,0.0};
//        return InterpolatedEfield;
//    }
//
...
//        InterpolatedEfield += Position[PointIndex[vertex_no][0]][PointIndex[vertex_no][1]][PointIndex[vertex_no][2]] * BaryCoord[vertex_no]);
//    }
//
...
//// This function interpolates regularly spaced grid points of the TPC and stores them in a std::vector (can later be used in the WriteRootFile function)
//std::vector<ThreeVector<float>> InterpolateMap(const std::vector<std::vector<std::vector<ThreeVector>>>& Position, const xDelaunay& Mesh, const TPCVolumeHandler& TPC)
//{
//    // Initialize output data structure
...
//
//                // Fill displacement map
//                EMap.push_back(EInterpolateCGAL(Position,Mesh,Location));
//            } // end zbin loop
//        } // end ybin loop
//    } // end ybin loop
//
